...
use super::uri_resolver_like::UriResolverLikeVariant;

pub extern "C" fn create_recursive_resolver(entries: *const UriResolverLikeVariant, len: usize) -> *mut RecursiveResolver {
  let mut uri_resolvers: Vec<UriResolverLike> = Vec::new();
  
  for i in 0..len {
    let entry_ptr = unsafe { entries.offset(i as isize) };
    let entry = instantiate_from_ptr(entry_ptr as *mut UriResolverLikeVariant);
