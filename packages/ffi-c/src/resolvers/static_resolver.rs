...
use super::uri_package_or_wrapper::UriPackageOrWrapperVariant;

#[no_mangle]
pub extern "C" fn create_static_resolver(entries: *const UriPackageOrWrapperVariant, len: usize) -> *mut StaticResolver {
  let mut uri_map: HashMap<String, UriPackageOrWrapper> = HashMap::new();
  for i in 0..len {
    let entry_ptr = unsafe { entries.offset(i as isize) };
    let entry = instantiate_from_ptr(entry_ptr as *mut UriPackageOrWrapperVariant);
