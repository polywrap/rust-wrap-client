...
impl AnyManifest {
    ...
    pub fn schema_file(&self) -> String {
        match self {
            AnyManifest::PolywrapManifest010(_) => "schemas/0.1.0.json".to_string(),
            AnyManifest::PolywrapManifest020(_) => "schemas/0.2.0.json".to_string(),
        }
    }

    pub fn is_latest(&self) -> bool {
        match self {
            AnyManifest::PolywrapManifest010(_) => false,
            AnyManifest::PolywrapManifest020(_) => true,
        }
    }

    pub fn get_latest(&self) -> Result<PolywrapManifest, polywrap_core::error::Error> {
        match self {
            AnyManifest::PolywrapManifest010(_) => Err(polywrap_core::error::Error::ManifestError(
